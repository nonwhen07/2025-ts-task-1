...
// export const catKeyName: string = PlantCategory[1];
export const catKeyName: string = PlantCategory[PlantCategory.LargePlant];
console.log('catKeyName:', catKeyName); // LargePlant
...
*/
//做法是先做可選部分，再組裝必選{ id: number; title: string }
export type UpdateProduct = { id: number; title: string } & Partial<
...
export const submitProduct = (
  ...
  if (type === 'create') {
    ...
    const updatedProduct = product as UpdateProduct;
    return `更新商品成功：${updatedProduct.id}`;
  }
};
